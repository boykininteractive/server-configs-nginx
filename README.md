...
sudo git clone https://github.com/boykininteractive/server-configs-nginx.git nginx
...
* Creating a new site
  ```bash
  $ sudo cp templates/example.com.conf .actual-hostname.conf
  $ sudo sed -i 's/example.com/actual-hostname/g' .actual-hostname.conf
  ```

* Enabling a site
  ```bash
  $ sudo mv .actual-hostname.conf actual-hostname.conf
  ```
